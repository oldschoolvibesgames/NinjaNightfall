...
public class FlyingChaseEnemy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_attackTimer >= attackCancelTime &&
            _state != State.Idle &&
    ...
    private bool IsWallAhead(Vector2 direction)
    {
        RaycastHit2D hit = Physics2D.Raycast(transform.position, direction, wallDetectionDistance, obstacleLayers);
    ...
    private bool IsPlayerVisible()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(transform.position, direction.normalized, distance, obstacleLayers);
        return hit.collider == null;
    }
    ...
    private void UpdatePlayerVision()
    {
        ...
        _playerTarget = distance <= visionRange && IsPlayerVisible();
        ...
        if (_hoverTimer <= 0f)
        {
            ...
            switch (selected)
            {
                ...
                case AttackType.DirectAttack:
                    ...
                    break;
                case AttackType.SweepAttack:
                    _state = _playerTarget ? State.SweepAttack : State.Returning;
                    break;
                case AttackType.DoubleDirectAttack:
                    if (_playerTarget)
                    {
